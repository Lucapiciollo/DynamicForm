...
export function GetErrorForm(formGroup: FormGroup = new FormGroup({}), formName: string): Array<string> {
    try {
        if (formName != null && !formGroup.get(formName)?.valid && formGroup.get(formName)?.dirty) {
            return Object.keys(formGroup!.get(formName)!.errors!).map(e => {
                try {
                    ...
                    if (formGroup.get(formName)!.errors![e].requiredLength)
                        msg = (ErrorMessage as any).replace("{0}", formGroup.get(formName)!.errors![e].requiredLength);
...
export function GetErrorFormControl(formControl: FormControl): Array<string> {
    try {
        if (formControl && !formControl.valid && formControl.dirty) {
            return Object.keys(formControl.errors!).map((e) => {
                try {

                    let msg = (ErrorMessage as any)[e];
                    if (formControl.errors![e].requiredLength)
                        msg = (ErrorMessage as any).replace("{0}", formControl.errors![e].requiredLength);
                    else if (formControl.errors![e].min) {
                        msg = (ErrorMessage as any).replace("{0}", formControl.errors![e].min);
                    }
                    else if (formControl.errors![e].max) {
                        msg = (ErrorMessage as any).replace("{0}", formControl.errors![e].max);
                    }
                    else if (formControl.errors![e].minCurrency) {
                        msg = (ErrorMessage as any).replace("{0}", formControl.errors![e].minCurrency);
                    }
                    return msg
                } catch (e) { }
            })
